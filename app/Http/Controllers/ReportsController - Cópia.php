...
//		echo "begin:" . $begin . " end: ". $end . "ServiceStart:". $ipt['serviceStart'];
//		echo "date: ".date('Y-m-d', strtotime($ipt['serviceStart']));

		if ($ipt['accountNumber']== "") 		
//		$act = Account::orderBy('accountNumber', 'asc')->with('actStatus')->with('field')->with('accounting')->with('salesmen');
		$act = Account::orderBy('accountNumber', 'asc')->with('actStatus')->with('field')->with('accounting')->with('salesmen');
		else

		$act = Account::where('accountNumber', $ipt['accountNumber'])->orderBy('accountNumber', 'asc')->with('actStatus')->with('field')->with('accounting')->with('salesmen');
        ...
        if ($begin) {
			$sBegin = strtotime(date('Y-m-d', strtotime($ipt['serviceStart'])));
//			echo "date-begin-input:".date('Y-m-d', strtotime($ipt['serviceStart']));
		...
		if ($end) {
			$sEnd = strtotime(date('Y-m-d', strtotime($ipt['serviceEnd'])));
//			echo "date-end-input:".date('Y-m-d', strtotime($ipt['serviceEnd']));
		}
		else
			$sEnd = null;
        ...
        if ($date_begin) {
			$sdStart = strtotime(date('Y-m-d', strtotime($ipt['startDate'])));
		}
		if ($date_end) {
			$sdEnd = strtotime(date('Y-m-d', strtotime($ipt['endDate'])));
		}

/*
 
       if ($begin) {
 			$dts = date('Y-m-d', strtotime($ipt['serviceStart']));
			$act->whereHas('field', function ($query) use ($dts) {  	
//			$query->where('service_date', '>=', $dts );
			$query->whereRaw("service_date = '". $dts ."'");	
		...
		if ($end) {
 			$dte = date('Y-m-d', strtotime($ipt['serviceEnd']));
 			echo "dte:".$dte;
			$act->whereHas('field', function ($query) use ($dte) {  			
//			$query->where('service_date', '<=', $dte );
			$query->whereRaw("service_date = '". $dte ."'");	
		});
		}
*/
		$act->whereHas('field', function ($query)  {  			
		});
       if (!$begin && !$end) {
			$act->whereHas('field', function ($query)  {  			
		});
...
/*
        ...
        if (isset($ipt['startDate']) && $ipt['startDate'] != '') {
 			$dts = date('Y-m-d', strtotime($ipt['startDate']));
			$act->whereHas('actInfo', function ($query) use ($dts) {
//				$query->where('date_begin', '>=', $dts);
			$query->whereRaw("date_begin >= '". $dts ."'");	
			});
		}
		if (isset($ipt['endDate']) && $ipt['endDate'] != '') {
 			$dte = date('Y-m-d', strtotime($ipt['endDate']));
			$act->whereHas('actInfo', function ($query) use ($dte) {
//				$query->where('date_end', '<=', $dte);
			$query->whereRaw("date_end <= '". $dte ."'");	
			});
		}
...
//		}
		...
		if (isset($ipt['salesm']) && $ipt['salesm'] != 'default') {
...
//		$return['act'] = $act->skip($skip)->take(10)->get();
//		$skip += 10;
		...
		if ($date_begin =="")
		   $date_begin1 = '2008-01-01'; 
		else $date_begin1 = $date_begin;
		if ($date_end =="")
		   $date_end1 = '2075-01-01'; 
		else $date_end1 = $date_end;
		...
		$return['sdatei'] = $begin1;
		$return['sdatef'] = $end1;
		$return['bdate'] = $date_begin1;
		$return['edate'] = $date_end1;
  		$return['salesman'] = $ipt['salesm'];
		$return['office'] = $ipt['office'];
...
//			echo "<br>Account:".$a->accountNumber." FieldCount: ".count($a->field) ;
			$actStats = new \stdClass();
			$actStats->id = $a->accountNumber;
			$actStats->fieldCount = count($a->field);
//			echo "contField1:".$actStats->fieldCount;
			$actHours = [];
			$hours = 0;
			$costTotal = 0;
			$found = 0;
			...
			$contField = 0;
			$actStats->fieldCount = $contField;
			foreach ($a->field as $f) {
				$sDate = new \DateTime($f->service_date);
//				echo "<br>Read Service date:".$f->service_date;
				$xDate = strtotime(date('Y-m-d', strtotime($f->service_date)));
				
				if ($end ) 
					if ($xDate > $sEnd){ continue;}
				if ($begin )
				 	if  ($xDate < $sBegin) { continue;}


				$ai = \App\ActInfo::where('account_id', $a->id)->first();
				$dBegin = strtotime(date('Y-m-d', strtotime($ai->date_begin)));
				$dEnd = strtotime(date('Y-m-d', strtotime($ai->date_end)));
//				echo "date-begin:".date('Y-m-d', strtotime($ai->date_begin));
//				echo "date-end:".date('Y-m-d', strtotime($ai->date_end));

/*				if ($date_begin)
				   if ($dBegin < $sdStart){ continue;}
				if ($date_end) 
					if ($dEnd > $sdEnd){   continue;}
*/
				$found = 1;
				$contField++;
				$actStats->fieldCount = $contField;
//				echo "contField:".$contField."CF:".$actStats->fieldCount;
...
//
	...
	public function printCarPg($id = null,$input_skip,$account, $sdatei,$sdatef,$bdate,$edate,$salesman,$office){
		set_time_limit(0);
		ini_set('memory_limit', '3.0G');
		\DB::enableQueryLog();
        ...
        $begind = (isset($sdatei) ?  str_replace('/', '-',$sdatei) : null);
		$end = (isset($sdatei) ? str_replace('/', '-', $sdatef) : null);
		
        $date_begin = (isset($bdate) ?  str_replace('/', '-',$bdate) : null);
		$date_end = (isset($edate) ? str_replace('/', '-', $edate) : null);

        $xbegind = (isset($sdatei) ?  str_replace('-', '/',$sdatei) : null);
		$xend = (isset($sdatei) ? str_replace('-', '/', $sdatef) : null);
		
        $xdate_begin = (isset($bdate) ?  str_replace('-', '/',$bdate) : null);
		$xdate_end = (isset($edate) ? str_replace('-', '/', $edate) : null);

 //       echo "<br>".$xbegind."*".$xend."*".$xdate_begin."*".$xdate_end;
        

		$act = Account::orderBy('accountNumber', 'asc')->with('actStatus')->with('field')->with('accounting')->with('salesmen');

		if ($account > 0) {
			$act->where('accountNumber', $account);
		}


//        if ($begind && !$end) {
//            $act->whereHas('field', function ($query) use ($begind) {
//  				$query->where('service_date', '>=', date('Y-m-d', strtotime($begind)));
//			});
//		}
//		if (!$begind && $end) {
//            $act->whereHas('field', function ($query) use ($end) {
//				$query->where('service_date', '<=', date('Y-m-d', strtotime($end)));
//			});
//		}
//		if ($begind && $end) {
//           $act->whereHas('field', function ($query) use ($begind,$end) {
//				$query->whereBetween('service_date', [date('Y-m-d', strtotime($begind)), date('Y-m-d', strtotime($end))]);
//			});
//		}
        ...
        if ($begind) {
			$sBegin = strtotime(date('Y-m-d', strtotime($xbegind)));
		} 
		else
			$sBegin = null;

		if ($end) {
			$sEnd = strtotime(date('Y-m-d', strtotime($xend)));
		}
		else
			$sEnd = null;

       if ($begind) {
 			$dts = date('Y-m-d', strtotime($xbegind));
			echo "DATE WHERE:".$dts;
			$act->whereHas('field', function ($query) use ($dts) {  	
			$query->where('service_date', '>=', $dts );
        ...
        if ($date_begin) {
			$sdStart = strtotime(date('Y-m-d', strtotime($xdate_begin)));
		}
		if ($date_end) {
			$sdEnd = strtotime(date('Y-m-d', strtotime($xdate_end)));
		}




		if (isset($office) && $office != 'default') {
			$act->whereHas('office', function ($query) use ($office) {
				$query->where('office_id', $office);
			});

		}
		if (isset($salesm) && $salesm != 'default') {
			$act->whereHas('salesmen', function ($query) use ($salesm) {
				$query->where('salesmen_id', $salesm);
			});
		}
...
//		echo "count:".$countAcc;
		...
		$return['sdatei'] = $sdatei1;
		$return['sdatef'] = $sdatef1;
		$return['bdate'] = $bdate1;
		$return['edate'] = $edate1;
...
//		}
        ...
        $act = $act->chunk(5, function ($act) use (&$tmp, $status, $offices, $sales, $notes,$dateend,$dwEnd) {
            foreach ($act as $a) {
				$temp = new \stdClass();
				$temp->actNum = $a->accountNumber;
				$temp->actName = $a->actName;
