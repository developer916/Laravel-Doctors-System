...
//			}
        ...
        if (isset($ipt['exp_sort']) && $ipt['exp_sort'] == 'accountNumber') {
        	$act = $act->orderBy('accountNumber', 'ASC');
    	}
    	else
    	{
    		$act = $act->orderBy('actName', 'ASC');
    	}	

        
		$act = $act->chunk(200, function ($act) use (&$tmp, $status, $offices, $salesmen, $salesmen_ids) {
			foreach ($act as $a) {
				$temp = new \stdClass();
...
//			die($act1);
            ...
            $return['lastService'] = \App\FieldService::where('account_id', $act1)->orderby('service_date', 'DESC')->first();
        }

		if ( !$return['lastService'] )
		{
			return view('errors.no-field-activity');
		}
     
		if (!empty($return['lastService'])) {
            $return['lastService']->techName = \App\Technician::where('id', $return['lastService']->tech_id)->first();
//   troquei account_id para old_number pq não estava acessando a conta correta
//            $act = \App\Account::where('accountNumber', $return['lastService']->account_id)->with('actStatus')->first();
            $act = \App\Account::where('id', $return['lastService']->account_id)->with('actStatus')->first();

//            $act = \App\Account::where('account_id', $return['lastService']->old_number)->with('actStatus')->first();
		    $return['act'] = $act;
...
//           $return['field'] = \App\FieldService::where('old_number', $return['lastService']->old_number)->with('fieldData')->orderby('service_date', 'DESC')->get();
           ...
           $fields = \App\FieldService::where('old_number', $return['lastService']->old_number);


           $return['field'] = $fields->with('fieldData')->with('fieldData.material')->orderby('service_date', 'DESC')->get() ;
...
//			dd($return['field']);
            ...
            $return['field'] = \App\FieldService::where('old_number', $id)->with('fieldData')->orderby('service_date', 'DESC')->get();
            $return['fieldCount'] = 0;
...
//        $return['id'] = $id;
	...
	public function getMaterialInventory(){
        ...
        $return['office'] = (isset($ipt['office']) ? $ipt['office'] : "" );
        $return['beginDate'] = (isset($ipt['inv_date_beg']) ?  date('Y-m-d',strtotime($ipt['inv_date_beg'])) : "")  ;
        $return['endDate'] = (isset($ipt['inv_date_end']) ?  date('Y-m-d',strtotime($ipt['inv_date_end'])) : "")  ;
        $return['mat_filter'] = (isset($ipt['mat-auto-lookup']) ?$ipt['mat-auto-lookup'] : "") ;

        $order1 = (isset($ipt['sort1']) ? $ipt['sort1'] : "" );	
        $order2 = (isset($ipt['sort2']) ? $ipt['sort2'] : "" );	
        $order3 = (isset($ipt['sort3']) ? $ipt['sort3'] : "" );	

        $query1 = 'select i.id, code,name,units, abvr, officeName,begin_amount,end_amount,purchased_amount,i.cost,month,trans from materials as m, offices as o, invantories as i ';	
		$query2 = 'where m.id = i.mat_id and o.id = i.office_id';
		if ($return['office'] && $return['office'] != 'default' )
			$query2 .= " and o.id = " . $return['office'];
		if ($return['beginDate'] && $return['beginDate'] != '1970-01-01' )
			$query2 .= " and month >= '" . date('Y-m-d',strtotime($return['beginDate'])). "'";
		if ($return['endDate']  && $return['endDate']  != '1970-01-01')
			$query2 .= " and month <= '" . date('Y-m-d',strtotime($return['endDate'])) . "'";
		if ($return['mat_filter'])
			$query2 .= " and name = '" . $return['mat_filter'] . "'";

		$query3 = ' order by '.$order1." , ".$order2." , ".$order3;	

		$materials3 = [];
		$materials3 = \DB::select($query1 . $query2 . $query3 );
		$return['material3'] = $materials3;
 	

		$return['bc'] = 'Material Reference List';
		$return['wide'] = true;
		$return['total'] = 0;
//		dd($return['material2'][0]['office_inventory']);
		return view('reports.materialInv', $return)
			->withEncryptedCsrfToken(Crypt::encrypt(csrf_token()));
	}

	public function getMaterialInventorySave(){

		$return['material'] = Material::with('inventory')->whereHas('inventory', function ($query)  {
			})->orderBy('code', 'ASC')->get();
//		$inventory2 = Material::with('inventory')->whereHas('inventory', function ($query)  {
//			})->orderBy('name', 'ASC')->get();


//		$inv = Invantory::with('invMaterial')->whereHas('invMaterial', function ($query)  {
//				$query->orderBy('code', 'ASC');
//			})->get();
//		$inv2 = Invantory::with('invMaterial')->whereHas('invMaterial', function ($query)  {
//				$query->orderBy('code', 'ASC');
//			})->get();

       foreach ($return['material'] as $mat) {

            $invCount = count($mat->inventory);
            

            foreach ($mat->inventory as $inv) {
				$invlist = new \stdClass();
	            $invlist->code = $mat->code;
	            $invlist->id = $mat->id;
	            $invlist->name = $mat->name;
	            $invlist->primary = $mat->primary;
	            $invlist->units = $mat->units;
	            $invlist->mcost = $mat->cost;

	            $invlist->office_id = $inv->office_id;
	            $invlist->mat_id = $inv->mat_id;
	            $invlist->begin_amount = $inv->begin_amount;
	            $invlist->end_amount = $inv->end_amount;
	            $invlist->purchased_amount = $inv->purchased_amount;
	            $invlist->cost = $inv->cost;
	            $invlist->updated_at = $inv->updated_at;

				$return['report'][] = $invlist;
...
//		dd($return);
	...
	public function printCpr($id){
		if ( !is_numeric($id) )
		{
			return view("errors.cpr-all-no-access");
		}
...
//        $models = Model::findMany([1, 2, 3]);
        ...
        foreach ($return['act'] as $act) {
			...
			$actStats->actName = $act->actName;
			$actStats->accountNumber = $act->accountNumber;

            $actStats->location = \App\Location::where('id', $act->location_id)->first();
			$actStats->location->state = \App\State::where('id', $actStats->location->state_id)->select('abvr')->first();
			...
			//PASTE BLOCK
			$actStats->actType->info = \App\Type::where('id', $act->actType->types_id)->select('name')->first();
			
			$actStats->actTax->info = \App\TaxDistrict::where('id', $act->actTax->taxDistrict)->select('name', 'id_code', 'percent')->first();
	//        dd($return['act']->actTax);
			
			
			$actStats->actTerms->term = \App\Term::where('id', $act->actTerms->term_id)->select('name')->first();
			$actStats->actStatus->name = \App\Status::where('id', $act->actStatus->status_id)->select('code')->first();
	//        $return['act']->actStatus->statusDate = $this->fixDates($return['act']->actStatus->statusDate);
			$actStats->accounting->budget = number_format($act->accounting->budget, 2);
			$actStats->accounting->yearlyAmount = number_format($act->accounting->yearlyAmount, 2);
			$actStats->accounting->billing = number_format($act->accounting->billing, 2);
			$actStats->accounting->initialStart = number_format($act->accounting->initialStart, 2);
			$actStats->accounting->initialBudget = number_format($act->accounting->initialBudget, 2);
			//END PASTE BLOCK
			...
			//END TEMP BLOCK
			array_push($return['acts'], $actStats);
...
//        dd($return['acts']);
	...
	public function printCar($id = null){
        ...
        $begin = (isset($ipt['serviceStart']) ?  str_replace('/', '-',$ipt['serviceStart']) : null);
		$end = (isset($ipt['serviceEnd']) ? str_replace('/', '-', $ipt['serviceEnd']) : null);
		
        $date_begin = (isset($ipt['startDate']) ?  str_replace('/', '-',$ipt['startDate']) : null);
		$date_end = (isset($ipt['endDate']) ? str_replace('/', '-', $ipt['endDate']) : null);

		if ($ipt['accountNumber']== "") 		
			$act = Account::orderBy('actName','ASC')->with('salesmen');
		else
			$act = Account::where('accountNumber', $ipt['accountNumber']);



        if ($begin) {
			$sBegin = date('Y-m-d', strtotime($ipt['serviceStart']));
//			echo "date-begin-input:".date('Y-m-d', strtotime($ipt['serviceStart']));
		} 
		else
			$sBegin = null;

		if ($end) {
			$sEnd = date('Y-m-d', strtotime($ipt['serviceEnd']));
			$sEndNextDay = date('Y-m-d', strtotime($ipt['serviceEnd'].' + 1 days'));
		}
		else
			$sEnd = null;
		...
		if ($date_end) {
			$sdEnd = date('Y-m-d', strtotime($ipt['endDate']));
			$sdEndNextDay = date('Y-m-d', strtotime($ipt['endDate'].' + 1 days'));
		}
		else
			$sdEnd = null;
		...
		if (isset($ipt['endDate']) && $ipt['endDate'] != '') {
			...
			$act->whereHas('actInfo', function ($query) use ($ipt,$sdEndNextDay) {
				$query->where('date_end', '<=', $sdEndNextDay);
			});
		}
		else
		{
...
//		}
		if ($end) {
//			echo "#".$sEndNextDay."#";
            $act->whereHas('field', function ($query) use ($ipt,$sEndNextDay,$sBegin) {
				$query->whereBetween('service_date', [$sBegin, $sEndNextDay]);
			});
		}
		if (!$begin && !$end){
           $act->whereHas('field', function ($query) use ($ipt) {
		...
		if (isset($ipt['salesm']) && $ipt['salesm'] != 'default') {  // salesman
			$act->whereHas('salesmen', function ($query) use ($ipt) {
				$query->where('salesmen_id', $ipt['salesm']);
			});
		}



//		echo 'Salesman:'.$ipt['salesm'];


		
//        echo 'before acct get: TIME:'.date('H:i:s');

//        $qtyrec = $act->count();
//        echo "Qty records read:".$qtyrec;


//		$return['act'] = $act->toSql();
		$return['act'] = $act->get();
//dd ($return['act']);
		...
		if ($renewEnd == 'Till now') {
			$renew = 'No End Date set.';
		}
		else			
		if ($renewEnd != 'Till now') {
			$renew = $renewEnd;
		...
		else	$treatDate = 'No Treatment dates set.';
		...
		$summary->tech = (isset($ipt['salesm']) && $ipt['salesm'] != 'default' ? $ipt['salesm'] : 'No Salesman selected');
		$summary->altCost = 0;
		$return['status'] = $allStatus;
		$return['summaryreport'] = $ipt['summary'];



		if (isset($ipt['accountNumber']) && $ipt['accountNumber'] != '') {
			$return['account'] = $ipt['accountNumber'];
		}
		else
			$return['account'] = '0';	
		...
		foreach ($return['act'] as $a) {
			...
			$fs = \App\FieldService::where('account_id', $a->id);
        	if ($begin) {
        		$fs->where('service_date','>=',$sBegin);	
        	}
        	if ($end) {
        		$fs->where('service_date','<=',$sEnd);	
        	}

        	$fs->orderBy('service_date', 'DESC');
        	$fsc = $fs->count();
        	$fsr = $fs->get();
			$actStats->fieldCount = $fsc;
			$actHours = [];
			$hours = 0;
			$costTotal = 0;
			$d1 = new \DateTime("2075-01-01");
			$d2 = new \DateTime("2008-01-01");
//			echo "<br>".$a->accountNumber."fieldCount:".$actStats->fieldCount;
...
//				echo "<br>Read Service date:".$f->service_date."old_number;".$f->old_number;

				$ai = \App\ActInfo::where('account_id', $a->id)->first();
				$dEnd = strtotime(date('Y-m-d', strtotime($ai->date_end)));


                $fdcount = \App\FieldData::where('field_service_id', $f->id)->count();
                $f->data = \App\FieldData::where('field_service_id', $f->id)->get();
				$fieldChem = 0;

				$curmonth = $sDate->format('m');
	...
	//				echo "<br>Read field_data:".$fd->id;
					...
					if ($sst->mat) {
						...
						$sst->matTotal = $foo;
						$costTotal += $sst->cost * $sst->qty;
						$fieldChem += $sst->cost * $sst->qty;
...
//						$fieldChem += round($sst->cost * $sst->qty, 2);
			...
			if ($actStats->fieldCount){
//			    $noMonths = $d1->diff($d2)->m + ($d1->diff($d2)->y * 12);
				$noMonths = $countmonths;
				...
				$actStats->avgHours = ($actStats->fieldCount > 0 ? $hours / $actStats->totalMonths : 'none');
...
//					$actStats->altCost = number_format(($actStats->costTotal + ($actStats->totalHours * 80)) / $actStats->totalMonths, 2);
					$actStats->altCost = number_format(($actStats->avgChem*2) + ($actStats->avgHours * 80), 2);
...
//				echo "fieldCount-again:".$actStats->fieldCount."*".$return['actStats']->fieldCount;
...
//
    ...
    public function getExp(){
		...
		$dateStart = (isset($ipt['exp_start']) ? str_replace('/', '-', $ipt['exp_start']) : null);
		$dateend = (isset($ipt['exp_end']) ? str_replace('/', '-', $ipt['exp_end']) : null);

		$act = Account::whereHas('actStatus.status', function ($query) {
			$query->where('code', 'NOT LIKE', 'C%');
		})
		->with('office')
		->with('actInfo')
		->with('accounting')
...
//		}
		...
		if (isset($ipt['exp_end']) && $ipt['exp_end'] != '' && (!isset($ipt['exp_start']) || $ipt['exp_start'] == '')) {
			$act = $act->whereHas('actInfo', function ($query) use ($ipt) {
				$query->where('date_end', '=', date('Y-m-d', strtotime($ipt['exp_end'])));
			});
		}
		else if (isset($ipt['exp_end']) && $ipt['exp_end'] != '' && isset($ipt['exp_start']) && $ipt['exp_start'] != '') {
			$act = $act->whereHas('actInfo', function ($query) use ($ipt) {
				$query->whereBetween('act_info.date_end', array(date('Y-m-d', strtotime($ipt['exp_start'])), date('Y-m-d', strtotime($ipt['exp_end']))) );
...
//			$inputdate = trim(date("Y-m-d", strtotime($ipt["exp_end"])));		
//			$act = $act->whereHas('actInfo', function ($query) use ($ipt,$inputdate) {
//				$query->whereDate('date_end', '<=','$inputdate');
//			});
//		}



		$dwEnd   = strtotime(date('Y-m-d', strtotime('01/01/1900')));
		$dwStart = strtotime(date('Y-m-d', strtotime('01/01/1900')));

 		if ($dateend) {
 
			$dwEnd = strtotime(date('Y-m-d', strtotime($ipt['exp_end'])));
		}

		if ($dateStart)
		{
			$dwStart = strtotime(date('Y-m-d', strtotime($ipt['exp_start'])));
		}
        ...
        if (isset($ipt['exp_type']) && $ipt['exp_type'] != '') {
            ...
            $act = $act->whereHas('actStatus', function ($query)  {
				//$query->where('status_id','=' , 0);
				$query->whereNotIn('status_id', [4,5,6,7,8,10]);
			});
        ...
        $wcount = 0;
        $act = $act->chunk(5, function ($act) use (&$tmp, $status, $offices, $sales, $notes,$dateend,$dwEnd, $dateStart, $dwStart, $wcount) {
            ...
            foreach ($act as $a) {
				...
				$wwDate = strtotime($a->actInfo->date_end);
            	$xDate = strtotime(date('Y-m-d', $wwDate));

            	$infoDateStart = strtotime($a->actInfo->date_end);
            	$xInfoDateStart = strtotime(date('Y-m-d', $infoDateStart));

 	          	if ($dateend){
 	           		if ($xDate > $dwEnd){ continue;}
            	}

            	if($dateStart)
				{
					if($xInfoDateStart < $dwStart) { continue; }
				}
                ...
                $staterow = \App\Location::where('id', $a->location_id)->first();                    
                $temp->state = \App\State::where('id', $staterow->state_id)->pluck('abvr');                    

                $temp->zipcode = $contact->zipcode;
//				echo "teste:".$temp->phone2."testez:".$temp->phone;
                if (isset($a->office) && isset($a->office->office_id))
					$temp->office = $offices->find($a->office->office_id)->abvr;
				else {
				    $temp->office = 'Orphan';
				...
				//$temp->status = ($status->find($a->actStatus->status_id)->code == 'AC' ? 'Active' : $status->find($a->actStatus->status_id)->code);
				$temp->status = $status->find($a->actStatus->status_id)->code;
				$temp->budget = $a->accounting->budget;
