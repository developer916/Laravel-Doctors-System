...
//			}
	...
	public function getFieldActivity($id){
        ...
        if (empty($return['lastService'])) {
            $return['lastService'] = \App\FieldService::where('account_id', $act)->orderby('service_date', 'DESC')->first();
        }
...
//            $act = \App\Account::where('accountNumber', $return['lastService']->old_number)->with('actStatus')->first();
		    $return['act'] = $act;
...
//        
		...
		foreach ($return['act'] as $a) {
			...
			foreach ($a->field as $f) {
				...
				$dBegin = strtotime(date('Y-m-d', strtotime($f->date_begin)));
				$dEnd = strtotime(date('Y-m-d', strtotime($f->date_end)));
				if ($date_begin)
				   if ($xDate < $sdStart){ continue;}
				if ($date_end) 
					if ($xDate > $sdEnd){ continue;}
