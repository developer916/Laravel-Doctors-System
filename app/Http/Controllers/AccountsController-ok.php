...
//		dd($me);
		...
		public function getAccount($id){
            $return['act'] = Account::where('accountNumber', $id)->first();
			...
			$return['statBool'] = ($return['status']->status_id ? true : false);
			$return['status']->statusName = $statusName['code'] . '&nbsp;&mdash;&nbsp;' . $statusName['name'] . $combineAcct;
...
//        $return['info']->permit_expire = $this->fixDate( $return['info']->permit_expire );
			...
			foreach (Status::all() as $s) {
				$return['status']['status'][ $s['id'] ] = $s['code'] . ' &mdash; ' . $s['name'];
			}
...
//        unset($ipt['status_type']);
			...
			$maxId = Account::max('accountNumber');
