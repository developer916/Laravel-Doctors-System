...
//        dd($return['acts']);
	...
	public function printCar($id = null){
		...
		if ($ipt['accountNumber']== "") 		
			$act = Account::orderBy('accountNumber', 'asc')->with('salesmen')->with('field');
		else
			$act = Account::where('accountNumber', $ipt['accountNumber'])->with('field');
...
//		echo 'Salesman:'.$ipt['salesm'];
		...
		$return['act'] = $act->orderBy('actName','ASC')->get();
        ...
        echo 'before acct loop: TIME:'.date('H:i:s');
		foreach ($return['act'] as $a) {
			...
			foreach ($a->field as $f) {
		        echo 'before each field TIME:'.date('H:i:s');

				echo "<br>account:".$a->accountNumber."date:".$f->service_date;
...
//				echo "<br>Read Service date:".$f->service_date;
                ...
                echo '<br>fieldata count: '.$fdcount.'f->id'.$f->id;
...
//		}
        ...
        $wcount = 0;
        $act = $act->chunk(5, function ($act) use (&$tmp, $status, $offices, $sales, $notes,$dateend,$dwEnd,$wcount) {
