...
//		unset($ipt['query']['id']);
	...
	public function officeView()
	{
		...
		$ipt = \Input::All();
		$query = 'select first_name,middle_name,last_name,email,u.id ';
		$query .= ' from office_user as ou, users as u where ou.user_id = u.id ';
		$query .= ' and  ou.office_id = ' . $ipt['id'];
		$query .= ' order by last_name';
		$usersByOffice = \DB::select($query);
			
		$return['data'] = [];
		$return['data'] = $usersByOffice;
	...
	public function destroy()
	{
		...
		$query = 'select id,first_name,is_super_admin,is_dev from users where id = '.$id;
		$user = \DB::select($query); 
		$x = $user[0]->first_name;

		if (!$user[0]) {
			$ipt1 ['success'] = ['result' => 'bad', 'msg' => 'User not found.'];
		}
		$y = $user[0]->is_super_admin; 
		$z = $user[0]->is_dev; 
		if (!$y && !$z) {
			//check for tech
			$query = 'delete from permission_user where user_id = '.$id;
			$perm = \DB::delete($query); 
			$query = 'delete from office_user where user_id = '.$id;
			$perm = \DB::delete($query); 
			$query = 'delete from users where id = '.$id;
			$userd = \DB::delete($query); 

			$ipt1['success'] = ['result' => 'good', 'msg' => 'User deleted.'.$id];
		} else {
			$ipt1['success'] = ['result' => 'bad', 'msg' => 'Cant delete SuperAdmin or Development testing accounts.'];
		}
